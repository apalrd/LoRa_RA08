...
/* Return a 64-bit random number as a character string in hex */
const char * rand64()
{
  static char buf[17];
  
  /* Need 64-bits of random, so two longs */
  uint32_t key[2];
  //Seed *anything* unique
  randomSeed(micros());
  key[0] = random(2147483647);
  key[1] = random(2147483647);
  /* If you are using the RP2040 we can use the HW-seeded RNG instead: */
  key[0] = rp2040.hwrand32();
  key[1] = rp2040.hwrand32();

  /* Print it in hex */
  sprintf(buf,"%08X%08X",key[0],key[1]);
  buf[16] = 0;
  return buf;
}


/* Function to generate a random EUI64 set and AppKey */
void ra08_genkey()
{
  char buf[RA08_BUF_SIZE];

  /* New DEV EUI */
  sprintf(buf,"AT+CDEVEUI=%s\n",rand64());
  Serial.write(buf);
  Serial1.write(buf);
  ra08_resp(2);
  ...
  /* New APP EUI */
  sprintf(buf,"AT+CAPPEUI=%s\n",rand64());
  Serial.write(buf);
  Serial1.write(buf);
  ra08_resp(2);

  /* New APP KEY */
  sprintf(buf,"AT+CAPPKEY=%s%s\n",rand64(),rand64());
  Serial.write(buf);
  Serial1.write(buf);
  ra08_resp(2);
}
...
void setup() {
  ...
  /* Generate new random values and write them
   * You can comment this out to just read them back
   */
  ra08_genkey();
